...
- Flatten to (N, 160), normalise feature scales with `scipy.cluster.vq.whiten`, then cluster with `scipy.cluster.vq.kmeans2(k, iter=20, minit="points")`
...
Do **not** reuse `generate_trajectory_pair` from `preference_optimization/utils.py` - it has DPO-specific pairing, threshold, and pruning logic. The playground needs a focused `generate_samples` function in `guidance_gui/generate_samples.py`.
...
├── __main__.py                      # enables `python3 -m guidance_gui`
...
└── scripts/
    └── generate_prototypes.py       # scipy kmeans2 clustering of training trajectories
