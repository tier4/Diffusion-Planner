...
def test_synthetic_epdms_matches_autoware_cpp_aggregation():
    ...
    human = _snapshot(
        {**_METRICS, "no_at_fault_collision": 1.0, "lane_keeping": 0.0, "extended_comfort": 1.0}
    )
    ...
    human_filtered_weighted = (5.0 * 0.8 + 5.0 * 1.0 + 2.0 * 1.0 + 2.0 * 1.0 + 2.0 * 0.0) / 16.0
    ...
    assert torch.allclose(
        result.human_filtered_weighted_metrics, torch.tensor([human_filtered_weighted])
    )
