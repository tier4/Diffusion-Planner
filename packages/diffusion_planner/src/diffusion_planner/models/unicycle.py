...
def rot_2d_to_3d(rot: torch.Tensor) -> torch.Tensor:
    ...
    """
    # Every row is derived from `rot` by shape, never from a Python-side batch
    # size: baking `rot.shape` into a literal makes an ONNX trace batch-specific.
    upper = torch.cat([rot, torch.zeros_like(rot[..., :1])], dim=-1)
    last_row = torch.zeros_like(upper[..., :1, :])
    last_row = torch.cat(
        [last_row[..., :2], torch.ones_like(last_row[..., 2:])], dim=-1
    )
...
class UnicycleAccelCurvatureActionSpace(ActionSpace):
    ...
    def action_to_traj(
        ...
        # Built by stacking rather than allocating from `traj_history_xyz.shape`:
        # a literal batch size baked into an ONNX trace makes it batch-specific.
        z = traj_history_xyz[..., -1:, 2].expand_as(x)
        traj_future_xyz = torch.stack([x, y, z], dim=-1)
