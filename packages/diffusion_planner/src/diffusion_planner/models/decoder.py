...
class TrajectoryEncoder(nn.Module):
    """Encode one complete trajectory into a single token."""
    ...
    def forward(self, trajectory: torch.Tensor) -> torch.Tensor:
        """Encode a trajectory `(B, T, 4)` into one token `(B, H)`.

        Args:
            trajectory: Trajectory with shape `(B, T, 4)`, where each state is
                `[x, y, cos_yaw, sin_yaw]`.
...
class TrajectoryDecoder(nn.Module):
    """Decode the ego token into a clean trajectory prediction.

    The decoder predicts the ego vehicle only. Neighbors still reach the model as
    scene tokens through the encoder; they are inputs, not outputs.
    """
    ...
    def forward(
        ...
        """Predict the clean ego trajectory from a noisy flow state.

        Args:
            x: Ego trajectory with shape `(B, T, 4)`, where each state is
                `[x, y, cos_yaw, sin_yaw]` and `T` equals `TRAJECTORY_LENGTH`.
                The whole trajectory is encoded into one token before attention.
        ...
        Returns:
            Predicted clean ego trajectory with shape `(B, T, 4)`.
        """
        ...
        return output.reshape(-1, self.trajectory_len, TRAJECTORY_DIM)
