...
def draw_ego_vehicle(ax, inputs, show_future=True):
    ...
    if show_future and "ego_agent_future" in inputs:
        ...
        if np.any(valid_indices):
            ...
            ax.scatter(
                valid_future[:, 0], valid_future[:, 1], c=colors, alpha=0.5, s=20
            )
...
def draw_neighbor_agents(ax, inputs):
    ...
    for i in range(neighbors.shape[0]):
        ...
        color = (
            ["blue", "green", "purple"][vehicle_type] if vehicle_type < 3 else "blue"
        )
        ...
        if "neighbor_agents_future" in inputs:
            ...
            valid_indices = ~(
                (neighbor_future[:, 0] == 0) & (neighbor_future[:, 1] == 0)
            )
            if np.any(valid_indices):
    ...
    if future_scatter_x:
        ax.scatter(
            future_scatter_x, future_scatter_y, c=future_scatter_colors, alpha=0.5, s=8
        )
...
def draw_lanes(ax, inputs):
    ...
    if lane_lines:
        lc_lanes = LineCollection(
            lane_lines, colors=lane_colors, alpha=0.25, linewidths=1
        )
...
def draw_polygons_and_lines(ax, inputs):
    ...
    if "line_strings" in inputs:
        for i in range(inputs["line_strings"].shape[1]):
            ...
            is_road_border = line_string.shape[-1] > 3 and np.any(
                line_string[:, 3] > 0.5
            )
            if is_road_border:
                ...
                ax.plot(
                    line_string[:, 0], line_string[:, 1], color="orange", linewidth=1
                )
...
def visualize_inputs(
    ...
    if ax is not None and len(view_ranges) == 1:
        ...
        ego_x, ego_y, ego_state = draw_ego_vehicle(
            ax, inputs, show_future=show_ego_future
        )
        if show_neighbors:
