...
def test_rule_requires_exactly_one():
    ...
    assert (
        P.PartitionRule(regex=r"^(?P<partition>[^/]+/[^/]+)").partition_of("a/b/c/x")
        == "a/b"
    )
...
def test_discover_groups_pairs_and_filters(tmp_path):
    ...
    (
        tmp_path / "projA/mapX/auto/2026-01-03/t1/control_mode_4_intervals.json"
    ).write_text("{}")
    ...
    assert (
        s[0].sidecar_path is not None
        and s[0].rel_dir == "projA/mapX/manual/2026-01-01/t1/route_0"
    )
...
def test_discover_path_list_validation(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        P.discover(
            tmp_path, P.PartitionRule(depth=4), path_list=[str(tmp_path / "nope.npz")]
        )
    with pytest.raises(ValueError):
...
def test_inspect_tree_non_sidecar_jsons_counts_only_truly_orphan(tmp_path):
    ...
    rep = P.inspect_tree(
        tmp_path, P.PartitionRule(depth=4), include=["*/manual/*"], exclude=[]
    )
