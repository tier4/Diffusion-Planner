...
from planner_shards.data_pipeline import pack_shards as CLI
from planner_shards.data_pipeline import tar_shards as T
from planner_shards.data_pipeline.partition import pid_of
...
def test_pack_remove_gc_scrub_export_keyset(tmp_path):
    ...
    assert not any(
        p.name.startswith(pid_of("pB/mY/manual/2026-01-03"))
        for p in root.shards_dir.iterdir()
    )
...
def test_pack_source_namespace_override_allows_relocated_dataset_root(
...
def test_pack_rejects_workers_below_one(tmp_path, capsys):
    ...
    common = ["--source", str(src), "--dest", str(dst), "--partition-depth", "4"]
    assert (
        CLI.main(["pack", *common, "--base", "none", "--tag", "v1", "--workers", "0"])
        == 1
    )
