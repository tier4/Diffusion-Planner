...
from planner_shards import shard_plan as SP
...
def _all_occ(plan):
    out = []
    for slot, pad in zip(plan.slots, plan.padding, strict=False):
...
def test_pad_bound_and_slot_starvation_raise():
    with pytest.raises(PlanError, match="pad") as exc_info:
        SP.make_plan(
            {
                SP.ShardRef("p", "r", 0): np.arange(7000),
                SP.ShardRef("q", "r", 0): np.arange(1),
            },
            world_size=1,
            workers_per_rank=2,
            batch_size=64,
