...
from planner_shards.data_pipeline.errors import VersionExistsError
...
def test_layout_paths(tmp_path):
    ...
    assert (
        root.manifest_path_for("abc", "d1", "m1")
        == tmp_path / "manifest" / "abc@d1.m1.parquet"
    )
...
def test_gc_roots_and_gc(tmp_path):
    ...
    root.write_version(
        _version(
            "v2", {"p/a": _entry("p/a", "d2", "m1"), "p/b": _entry("p/b", "d3", "m3")}
        )
    )
    root.set_latest("v2")
