...
from planner_shards.data_pipeline.encoding import (
    arrays_bitexact,
    decode_sample,
    load_npz_bytes,
)
...
def test_pack_builds_version_bitexact(tmp_path):
    ...
    sum(t.num_rows for t in tables.values())
...
def test_pack_is_idempotent_and_detects_changes(tmp_path):
    ...
    changed = {
        p
        for p in v3.partitions
        if v3.partitions[p].data_rev != v2.partitions[p].data_rev
    }
...
def test_source_change_during_pack_fails(tmp_path, monkeypatch):
    ...
    with pytest.raises(SourceChangedError):
        PK.pack(_opts(src, dst, "v1"))
    assert DatasetRoot(dst).latest() is None and not list(
        (dst / "versions").glob("*.json")
    )
...
def test_remove_and_scrub(tmp_path):
    ...
    assert (
        "psim/loc_seed_1/manual/seed_1" not in v2.partitions and len(v2.partitions) == 3
    )
    rep = PK.scrub(dst, "v2")
...
def test_relation_move_does_not_overwrite_existing(tmp_path):
    ...
    assert (
        e2.data_rev == e1.data_rev
    )  # forced rebuild reproduced the identical revision
    after = rel.stat()
...
def test_sync_with_partition_filter_keeps_unselected(tmp_path):
    ...
    v2 = PK.pack(
        _opts(
            src, dst, "v2", base="v1", sync=True, partitions=["pA/mX/manual/2026-01-01"]
        )
    )
