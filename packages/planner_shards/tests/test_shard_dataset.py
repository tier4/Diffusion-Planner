...
from planner_shards import shard_dataset as SD
...
def packed(tmp_path_factory):
    ...
    ks = K.materialize_keyset(
        DatasetRoot(dst), "v1", "is_skipped IS NOT TRUE", tmp / "all.parquet"
    )
...
def test_output_contract_and_bitexact(packed):
    ...
    for key, sample in SD._debug_iter_with_keys(ds):
        assert (
            set(sample)
            == set(load_npz_bytes((src / f"{key}.npz").read_bytes())) - {"version"}
            and len(sample) == 17
        )
        assert arrays_bitexact(
            sample
            | {"version": load_npz_bytes((src / f"{key}.npz").read_bytes())["version"]},
...
def test_metadata_passthrough_is_opt_in(packed):
    ...
    assert (
        ds.meta_dictionary["project_id"][expected_value] == sample["meta"]["project_id"]
    )
