...
from planner_shards.data_pipeline import packer as PK
from planner_shards.data_pipeline.encoding import arrays_bitexact, load_npz_bytes
from planner_shards.data_pipeline.errors import PlanError
from planner_shards.data_pipeline.export import export
from planner_shards.data_pipeline.partition import PartitionRule
from planner_shards.data_pipeline.reader import ShardReader

LAYOUT = [
    ("pA/mX/manual/2026-01-01/t1/r", 30, "full"),
    ("pB/mY/manual/2026-01-02/t1/r", 8, "psim"),
]
...
def test_export_roundtrip_for_legacy_tools(packed, tmp_path):
    ...
    n = export(
        dst,
        "v1",
        "project_id = 'projA' AND timestamp < 1700000000000000000 + 5 * 300000000",
        out,
    )
    assert n == 5
    ...
    for k in man["keys"]:
        exported = dict(
            np.load(out / f"{k}.npz", allow_pickle=True)
        )  # legacy np.load works
        assert arrays_bitexact(
            exported, load_npz_bytes((src / f"{k}.npz").read_bytes())
        )
        js = json.loads((out / f"{k}.json").read_text())
        orig = json.loads((src / f"{k}.json").read_text())
        assert (
            js["is_skipped"] == orig["is_skipped"]
            and js["timestamp"] == orig["timestamp"]
        )
        assert (
...
def test_iter_seek_and_skim_both_exercised(packed):
    ...
    shard_info = next(
        (
            (ks, rs)
            for ks, rs in zip(by_shard.values(), by_shard_full.values(), strict=False)
            if len(ks) >= 2
        ),
        None,
    )
    ...
    with patch("planner_shards.data_pipeline.reader.SEEK_THRESHOLD", 0.5):  # noqa: SIM117
        # With threshold=0.5, selecting 1 key gives selectivity 1/N < 0.5 (for N >= 2)
        with patch(
            "planner_shards.data_pipeline.reader.T.iter_members",
    ...
    with patch("planner_shards.data_pipeline.reader.SEEK_THRESHOLD", 0.1):  # noqa: SIM117
        # With threshold=0.1, selecting all keys gives selectivity 1.0 >= 0.1 → skim
        with patch(
            "planner_shards.data_pipeline.reader.T.read_member",
            ...
            for k in seen:
                assert arrays_bitexact(
                    seen[k], load_npz_bytes((src / f"{k}.npz").read_bytes())
                )
