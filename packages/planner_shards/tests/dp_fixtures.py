...
def make_arrays(
    rng: np.random.Generator, *, small: bool = True
) -> dict[str, np.ndarray]:
    out = {}
    for name, (shape, dt) in _SHAPES.items():
        if dt == np.bool_:
            ...
            arr.reshape(-1)[: arr.size // 2] = (
                0.0  # padding-like zeros, as in real data
            )
            out[name] = arr
...
def write_sample(
    dir_: Path, stem: str, arrays: dict, sidecar: dict | None, *, compressed=True
):
