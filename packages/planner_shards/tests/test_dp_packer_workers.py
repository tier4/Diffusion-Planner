...
from planner_shards.data_pipeline.errors import PackWorkerError, PlanError
...
def _shard_digests(root, version):
    ...
    for e in version.partitions.values():
        ...
        for name in e.shards:
            out[f"{e.partition_id}/{name}"] = hashlib.sha256(
                (d / name).read_bytes()
            ).hexdigest()
    return out
...
def test_worker_failure_publishes_nothing_and_names_the_partition(
