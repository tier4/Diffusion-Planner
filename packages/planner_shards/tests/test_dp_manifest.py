...
from planner_shards.data_pipeline import manifest as M
from planner_shards.data_pipeline.sidecar import SIDECAR_FIELDS, parse_sidecar
...
def test_meta_rev_depends_only_on_sidecar_values():
    ...
    assert (
        M.meta_rev(M.rows_to_table(moved)) == a
    )  # index columns do not affect meta_rev
