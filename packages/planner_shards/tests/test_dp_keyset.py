...
from planner_shards.data_pipeline import keyset as K
from planner_shards.data_pipeline import packer as PK
from planner_shards.data_pipeline.errors import KeysetMismatchError, PlanError
...
LAYOUT = [
    ("pA/mX/manual/2026-01-01/t1/r", 10, "full"),
    ("pB/mY/manual/2026-01-02/t1/r", 6, "skip"),
]
...
def test_materialize_and_load(packed, tmp_path):
    ...
    out = K.materialize_keyset(
        root,
        "v1",
        "is_skipped IS NOT TRUE AND project_id = 'projA'",
        tmp_path / "ks.parquet",
    )
    ...
    with pytest.raises(PlanError):
        K.materialize_keyset(
            root, "v1", "project_id = 'nope'", tmp_path / "empty.parquet"
        )
    with pytest.raises(PlanError):
...
def test_keys_resolution_rejects_unknown_and_duplicates(packed, tmp_path):
    ...
    t = K.load_keyset(
        K.keyset_from_keys(root, "v1", good, tmp_path / "a.parquet"), root, "v1"
    )
    assert t.num_rows == 4
    with pytest.raises(PlanError):
    ...
    with pytest.raises(PlanError):
        K.keyset_from_keys(
            root, "v1", good + ["pA/does/not/exist"], tmp_path / "c.parquet"
        )
...
def test_version_binding(packed, tmp_path):
    ...
    ks = K.materialize_keyset(
        root, "v1", "is_skipped IS NOT TRUE", tmp_path / "ks.parquet"
    )
...
def test_load_rejects_duplicate_coordinates(packed, tmp_path):
    ...
    dup_table = pa.table(
        {
            "partition_id": pa.array(
                ["pA/mX/manual/2026-01-01", "pA/mX/manual/2026-01-01"]
            ),
    ...
    meta = {
        b"dp.version_tag": b"v1",
        b"dp.version_hash": root.version_hash("v1").encode(),
        b"dp.created_at": b"2026-09-02T00:00:00+00:00",
    }
...
def test_keyset_digest_is_stable_and_content_sensitive(packed, tmp_path):
    ...
    ks1 = K.materialize_keyset(
        root, "v1", "is_skipped IS NOT TRUE", tmp_path / "ks1.parquet"
    )
