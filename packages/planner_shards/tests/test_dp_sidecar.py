...
from planner_shards.data_pipeline import sidecar
from planner_shards.data_pipeline.errors import SidecarError
...
def test_parse_variants_yield_full_column_set(variant):
    ...
    if variant == "psim":
        assert (
            f["is_skipped"] is None
            and f["project_id"] is None
            and f["neighbor_count"] is None
        )
...
def test_sidecar_path_and_neighbor_ids(tmp_path):
    assert (
        sidecar.sidecar_path_for(tmp_path / "a/b_00000001.npz")
        == tmp_path / "a/b_00000001.json"
    )
    assert sidecar.neighbor_ids_of(
        json.dumps(make_sidecar("neighbor", 0)).encode()
    ) == [
        "id0a",
        "id0b",
    ]
