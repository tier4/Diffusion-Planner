...
from planner_shards.data_pipeline import encoding
from planner_shards.data_pipeline import tar_shards as T
from planner_shards.data_pipeline.defaults import (
    CHUNK_SIZE,
    MAX_PAD_FRACTION,
    READ_RETRIES,
    SEEK_THRESHOLD,
...
from planner_shards.data_pipeline.errors import IntegrityError
from planner_shards.data_pipeline.keyset import keyset_digest, load_keyset
from planner_shards.data_pipeline.manifest import read_manifest
from planner_shards.data_pipeline.versioning import DatasetRoot, Version
from planner_shards.shard_plan import (
    Chunk,
    Occurrence,
    Plan,
    ShardRef,
...
class ShardDatasetConfig:
    ...
    sample_meta_columns: tuple[
        str, ...
    ] = ()  # existing manifest columns only (spec §5.9)
...
def load_selection(
    ...
    for p, s, i in zip(pids, sids, idxs, strict=False):
        out.setdefault(
            ShardRef(str(p), version.partitions[str(p)].data_rev, int(s)), []
        ).append(int(i))
...
class ShardDataset(IterableDataset):
    def __init__(self, cfg: ShardDatasetConfig, plan: Plan | None = None):
        ...
        self.version = self.root.read_version(
            cfg.version
        )  # preflight: version resolves
        ...
        if cfg.sample_meta_columns:
            for _pid, e in sorted(self.version.partitions.items()):
                ...
                for col in cfg.sample_meta_columns:
                    ...
                    for v in sorted(
                        set(t.column(col).to_pylist()), key=lambda x: (x is None, x)
                    ):
    ...
    @functools.lru_cache(maxsize=64)  # noqa: B019 -- bounded per-worker cache, lives with the loader
    def _shard_table(
        self, shard: ShardRef
        ...
        t = read_manifest(
            self.root.manifest_path_for(e.pid, e.data_rev, e.meta_rev), columns=cols
        )
        ...
        if self.cfg.verify_reads:
            self._sha[shard] = dict(
                zip(
                    (int(i) for i in idx),
                    sub.column("payload_sha256").to_pylist(),
                    strict=False,
                )
            )
        if (
            ...
            for col in self.cfg.sample_meta_columns:
                codes = self._meta_codes.setdefault(col, {})
                for i, v in zip(idx, sub.column(col).to_pylist(), strict=False):
        ...
        return (
            path,
            {int(i): (int(o), int(s)) for i, o, s in zip(idx, off, size, strict=False)},
            int(mask.sum()),
        )
    ...
    def _retrying(self, fn, *args):
        ...
        raise IntegrityError(
            f"read failed after {self.cfg.read_retries} retries: {last}"
        )
        ...
        if chunk.n / max(n_members, 1) >= self.cfg.seek_threshold:
            ...
            try:
                ...
                for idx in sorted(remaining):
                    off, size = table[idx]
                    yield (
                        Occurrence(chunk.shard, idx, 0),
                        self._seek_read(path, off, size),
                    )
        else:
    ...
    def _decode(self, occ: Occurrence, payload: bytes) -> dict[str, np.ndarray]:
        if self.cfg.verify_reads:
            ...
            if hashlib.sha256(payload).digest() != self._sha[occ.shard][occ.index]:
                raise IntegrityError(
                    f"payload sha mismatch for {occ.shard} member {occ.index}"
                )
        try:
    ...
    def _slot_occurrences(
        self, worker: int, epoch: int
    ) -> Iterator[tuple[Occurrence, bytes]]:
        chunks, padding = self.plan.for_slot(self.cfg.rank, worker)
        cfg = self.cfg
        queue = (
            epoch_order(chunks, cfg.seed, epoch, cfg.rank, worker)
            if cfg.shuffle
            else list(chunks)
        )
    ...
    def _buffered(
        self, items: Iterator[tuple[Occurrence, bytes]], epoch: int, worker: int
    ):
        cfg = self.cfg
        if not cfg.shuffle:
            yield from items
    ...
    def _iter_occurrences(self) -> Iterator[tuple[Occurrence, dict[str, np.ndarray]]]:
        ...
        for occ, payload in self._buffered(
            self._slot_occurrences(worker, epoch), epoch, worker
        ):
...
def _debug_slot_keys(ds: ShardDataset, worker: int) -> list[str]:
    ...
    return [
        ...
        for o, _ in ds._buffered(
            ds._slot_occurrences(worker, int(ds._epoch.value)),
            int(ds._epoch.value),
            worker,
        )
    ]
