...
def build_chunks(selection: dict[ShardRef, np.ndarray], chunk_size: int) -> list[Chunk]:
    ...
    for shard in sorted(selection):
        ...
        for start in range(0, len(idx), chunk_size):
            chunks.append(
                Chunk(shard, tuple(int(i) for i in idx[start : start + chunk_size]))
            )
    return chunks
...
def pad_plan(
    ...
    for s, count in zip(slots, counts, strict=False):
        need = target - count
...
def epoch_order(
    chunks: list[Chunk], seed: int, epoch: int, rank: int, worker: int
) -> list[Chunk]:
