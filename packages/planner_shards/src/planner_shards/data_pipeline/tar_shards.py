...
from planner_shards.data_pipeline.encoding import MEMBER_EXT
...
class ShardWriter:
    ...
    def _open(self) -> None:
        ...
        self._fh = open(self._tmp, "wb")  # noqa: SIM115 -- held open across add() calls
        self._tar = tarfile.open(  # noqa: SIM115 -- closed in _roll()
            fileobj=self._fh, mode="w", format=tarfile.USTAR_FORMAT
        )
    ...
    def add(self, payload: bytes) -> MemberRecord:
        ...
        info.size, info.mtime, info.mode, info.uid, info.gid = (
            len(payload),
            0,
            0o444,
            0,
...
def iter_members(
    ...
    if expected_count is not None and count != expected_count:
        raise IntegrityError(
            f"member count mismatch: got {count}, expected {expected_count}"
        )
...
def read_member(fileobj: BinaryIO, offset: int, size: int) -> bytes:
    ...
    if len(data) != size:
        raise IntegrityError(
            f"short read at offset {offset}: {len(data)} of {size} bytes"
        )
    return data
