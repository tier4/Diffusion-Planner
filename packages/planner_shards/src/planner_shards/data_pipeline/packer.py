...
from planner_shards.data_pipeline import FORMAT_VERSION, PACKER_VERSION, encoding
from planner_shards.data_pipeline import partition as P
from planner_shards.data_pipeline import tar_shards as T
from planner_shards.data_pipeline import versioning as V
from planner_shards.data_pipeline.defaults import SHARD_SIZE_BYTES
from planner_shards.data_pipeline.errors import (
    EncodingError,
    IntegrityError,
    PackWorkerError,
    PlanError,
...
from planner_shards.data_pipeline.manifest import (
    ManifestRow,
    meta_rev,
    read_manifest,
    rows_to_table,
...
from planner_shards.data_pipeline.sidecar import (
    SIDECAR_FIELDS,
    is_rejected,
    neighbor_ids_of,
    parse_sidecar,
...
def _build_partition(job: WorkerJob) -> PartitionBuild:
    ...
    data_fp = hashlib.sha256(
        "\n".join(
            sorted(f"{s.key}\t{nsha.hex()}" for s, nsha, _ssha, _, _ in kept)
        ).encode()
    ).hexdigest()
    ...
    if base_entry and not opts.force and base_entry.data_rev == data_rev:
        ...
        return PartitionBuild(
            entry, None, manifest_path, None, False, rejected, missing
        )
    ...
    rng = np.random.default_rng(
        np.random.SeedSequence(
            [opts.seed, zlib.crc32(partition_id.encode()), FORMAT_VERSION]
        )
    )
    ...
    for i in order:
        ...
        if hashlib.sha256(npz_bytes).digest() != nsha:
            raise SourceChangedError(
                f"{s.key}: npz changed between discovery and build"
            )
    ...
    for s in samples:
        if (s.sidecar_path is None) != (not s.npz_path.with_suffix(".json").exists()):
            raise SourceChangedError(
                f"sidecar appeared/disappeared during pack: {s.key}"
            )
            ...
            pa.table(
                {
                    ...
                    "neighbor_ids": pa.array(
                        [v for _, v in relation], pa.list_(pa.string())
                    ),
                }
            ),
            rel_path,
    ...
    return PartitionBuild(
        entry, shards_dir, manifest_path, rel_path, False, rejected, missing
    )
...
def _verify_build(
    ...
    for sid, name in enumerate(shard_names):
        ...
        expect = sorted(
            per_shard.get(sid, []), key=lambda r: r["sample_index_in_shard"]
        )
        ...
        with open(shards_dir / name, "rb") as f:
            for (idx, off, size), r in zip(listed, expect, strict=False):
                ...
                if idx % 100 == 0 and not encoding.arrays_bitexact(
                    ...
                    raise IntegrityError(
                        f"{name}[{idx}]: spot decode differs from source"
                    )
        ...
        if b.build_shards_dir is not None:
            ...
            if target.exists():
                if sorted(p.name for p in target.iterdir()) != sorted(b.entry.shards):
                    raise IntegrityError(
                        f"existing revision dir {target} differs from build"
                    )
...
class _Progress:
    ...
    def __init__(
        self, total_partitions: int, total_samples: int, out=None, every_s: float = 5.0
    ):
                ...
                for fut in done:
                    ...
                    try:
                        ...
                        pending = sorted(
                            {
                                job.partition_id,
                                *(j.partition_id for j in inflight.values()),
                            }
                        )
                        ...
                        raise PackWorkerError(
                            f"partition {job.partition_id}: {e}"
                        ) from e
...
def pack(opts: PackOptions) -> V.Version:
    ...
    with V.writer_lock(root):
        ...
        if (
            ...
            and (
                base.rule_hash != opts.rule.rule_hash
                or base.source_namespace != namespace
            )
        ):
        ...
        if opts.sync and opts.path_list is not None:
            raise PlanError(
                "--sync requires a full source scan; cannot be used with --path-list"
            )
        groups = P.discover(
            opts.source, opts.rule, opts.include, opts.exclude, opts.path_list
        )
        ...
        if not groups and not opts.sync:
            raise PlanError(
                "nothing to pack: selection matched no partitions in source"
            )
        ...
        jobs = [
            _job_for(opts, build_dir, base, root, pid, samples)
...
def scrub(dest: Path, tag: str) -> dict[str, int]:
    ...
    for e in v.partitions.values():
        ...
        expect = {
            (r["shard_id"], r["sample_index_in_shard"]): r for r in table.to_pylist()
        }
