...
from planner_shards.data_pipeline import FORMAT_VERSION
from planner_shards.data_pipeline.defaults import ZSTD_LEVEL
from planner_shards.data_pipeline.errors import EncodingError, IntegrityError
...
def preflight(arrays: dict[str, np.ndarray]) -> None:
    for name, a in arrays.items():
        ...
        dt = a.dtype
        if (
            dt.kind not in _SUPPORTED_KINDS
            or dt.itemsize not in _SUPPORTED_ITEMSIZES[dt.kind]
        ):
            raise EncodingError(f"{name}: unsupported dtype {dt.str}")
        if dt.byteorder == ">" or (dt.byteorder == "=" and not _little_endian()):
            raise EncodingError(
                f"{name}: big-endian arrays are not supported ({dt.str})"
            )
...
def encode_sample(arrays: dict[str, np.ndarray]) -> bytes:
    preflight(arrays)
    contiguous = {
        k: v if v.ndim == 0 else np.ascontiguousarray(v) for k, v in arrays.items()
    }
