...
from planner_shards.data_pipeline.defaults import SEEK_THRESHOLD
from planner_shards.data_pipeline.duck import run_query
from planner_shards.data_pipeline.keyset import manifest_files
...
class ShardReader:
    ...
    def query(self, where: str, columns: list[str] | None = None) -> pa.Table:
        ...
        if columns:
            cols = ", ".join(
                f'"{c}"' if c in ("offset", "size") else c for c in columns
            )
        else:
            cols = "*"
        return self.execute(
            f"SELECT {cols} FROM read_parquet(?) WHERE {where} ORDER BY key",
            [self._files],
        )
    ...
    def iter(
        ...
        decode = (
            encoding.decode_for_training if training_view else encoding.decode_sample
        )
