"""pack_shards CLI (spec §4). Usage: python -m planner_shards.data_pipeline.pack_shards <cmd> ..."""
...
from planner_shards.data_pipeline import export as E
...
from planner_shards.data_pipeline.errors import PipelineError
...
def _add_rule(p: argparse.ArgumentParser) -> None:
    ...
    g.add_argument(
        "--partition-depth",
        type=int,
        help="partition = first N path components (no default)",
    )
    g.add_argument(
        "--partition-regex",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--base", required=True, help="base version tag, 'latest', or 'none'"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if a.cmd == "inspect":
            ...
            print(
                f"exported {E.export(a.dest, a.tag, a.where, a.out)} samples to {a.out}"
            )
        elif a.cmd == "keyset":
            root = V.DatasetRoot(a.dest)
            if a.where is not None:
                ...
                K.keyset_from_keys(
                    root, a.tag, json.loads(a.keys_json.read_text()), a.out
                )
            print(f"wrote {a.out}")
        return 0
    except (
