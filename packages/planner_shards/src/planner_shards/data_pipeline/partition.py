...
from planner_shards.data_pipeline.sidecar import sidecar_path_for
...
class PartitionRule:
    ...
    def partition_of(self, key: str) -> str:
        if self.depth is not None:
            ...
            if len(parts) - 1 < self.depth:  # key's last component is the frame stem
                raise PlanError(
                    f"key {key!r} shallower than partition depth {self.depth}"
                )
        ...
        return (
            m.group("partition")
            if "partition" in self._compiled_regex.groupindex
            else m.group(1)
        )
    ...
    def rule_hash(self) -> str:
        text = (
            f"depth={self.depth}" if self.depth is not None else f"regex={self.regex}"
        )
...
def _rel_npz_paths(source: Path, path_list) -> list[str]:
    ...
    for raw in path_list:
        ...
        try:
            ...
            raise ValueError(f"path outside source: {raw}") from None
        if not p.is_file():
...
def fingerprint(entries) -> str:
    lines = sorted(
        f"{k}\t{n.hex()}\t{s.hex() if s is not None else '-'}" for k, n, s in entries
    )
...
class InspectReport:
    ...
    def render(self) -> str:
        ...
        if self.outside_rule:
            lines.append(
                f"OUTSIDE RULE ({len(self.outside_rule)}): {self.outside_rule[:10]}"
            )
        return "\n".join(lines)
...
def inspect_tree(source: Path, rule: PartitionRule, include, exclude) -> InspectReport:
    ...
    for rel in npz_rels:
        ...
        rep.npz_depth_histogram[len(parts)] = (
            rep.npz_depth_histogram.get(len(parts), 0) + 1
        )
