...
def _write(
    out_path: Path, table: pa.Table, root: DatasetRoot, tag: str, where: str | None
) -> Path:
    if table.num_rows == 0:
...
def materialize_keyset(
    root: DatasetRoot, version_tag: str, where: str, out_path: Path
) -> Path:
...
def keyset_from_keys(
    root: DatasetRoot, version_tag: str, keys: list[str], out_path: Path
) -> Path:
