...
class DatasetRoot:
    ...
    def version_hash(self, tag: str) -> str:
        ...
        return hashlib.sha256(
            (self.versions_dir / f"{tag}.json").read_bytes()
        ).hexdigest()
...
class Journal:
    ...
    def advance(self, phase: str) -> None:
        ...
        if phase not in PHASES or (
            done and PHASES.index(phase) <= PHASES.index(done[-1])
        ):
        ...
        _write_atomic(
            self.path,
            json.dumps(
                {"build_id": self.build_dir.name, "phases": done + [phase]}
            ).encode(),
        )
...
def gc(root: DatasetRoot, dry_run: bool) -> list[Path]:
    ...
    if root.manifest_dir.exists():
        victims.extend(
            [p for p in sorted(root.manifest_dir.glob("*.parquet")) if p not in refs]
        )
