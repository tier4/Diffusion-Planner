...
def resolve_npz_roots(npz_root) -> list[Path | list[Path]]:
    ...
    ``--valid_set_list`` -- or an already-resolved list of paths (e.g. from
    ``site_discovery.discover_sites_from_json``, which does its own per-site grouping). A
    directory is returned as a one-element list; a pre-resolved list is returned verbatim.
