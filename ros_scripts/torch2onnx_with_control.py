...
def compare_outputs(torch_output, onnx_output):
    def _compare_pair(name, torch_arr, onnx_arr):
        print(f"\n{name} comparison:")
        print(f"  torch shape: {torch_arr.shape}")
        print(f"  onnx  shape: {onnx_arr.shape}")
        abs_diff = np.abs(torch_arr - onnx_arr)
        print(f"  Max diff: {abs_diff.max()}")
        print(f"  Mean diff: {abs_diff.mean()}")
        print(f"  Close? {np.allclose(torch_arr, onnx_arr, rtol=1e-03, atol=1e-05)}")

    _compare_pair("Prediction", torch_output[0], onnx_output[0])
    _compare_pair("Turn indicator", torch_output[1], onnx_output[1])

    if len(torch_output) > 2 and len(onnx_output) > 2:
        _compare_pair("Ego control", torch_output[2], onnx_output[2])
