...
from rlvr.grpo_trainer_batched import load_npz_data


def main():
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("--model", required=True, help="competent target-source model .pth (e.g. baseline)")
        ...
        for i, sp in enumerate(batch):
            ...
            if "ego_shape" in d:
                ...
                if not np.allclose(es, ego_shape, atol=1e-2):
                    raise ValueError(f"{sp} ego_shape {es.tolist()} != --ego_shape {ego_shape.tolist()}")
    ...
    print(f"  target = det of {args.model}; train curated (lr 5e-5) warm-started from the wounded model.")
