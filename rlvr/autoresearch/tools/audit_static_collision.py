...
def _synthesize_stopped_futures(
    nb_past: np.ndarray, future_len: int
) -> np.ndarray:
...
def _score_prediction(
    ...
    nb_fut_4 = np.concatenate(
        [nb_fut[..., :2], np.cos(yaw), np.sin(yaw)], axis=-1
    ).astype(np.float32)
...
def _draw_audit_figure(
    ...
    cog = (cx + math.cos(heading) * ego_wb / 2.0,
           cy + math.sin(heading) * ego_wb / 2.0)
    ...
    ax.add_patch(Rectangle(
        (-ego_L / 2, -ego_W / 2), ego_L, ego_W,
        lw=2.0, ec=ego_color, fc=ego_color, alpha=0.35,
        zorder=20, transform=t_rot,
    ...
    for i in range(nb_fut_4.shape[0]):
        ...
        ax.add_patch(Rectangle(
            ...
            zorder=15 if is_offender else 13, transform=t_rot,
        ))
    ...
    ax.plot([pt_e[0], pt_n[0]], [pt_e[1], pt_n[1]],
            "-", color=ego_color, lw=2.2, alpha=0.95, zorder=30)
    ax.plot([pt_e[0], pt_n[0]], [pt_e[1], pt_n[1]],
            "o", color=ego_color, ms=6, zorder=31,
    ...
    ax.annotate(
        ...
        fontsize=7, color=ego_color,
        ha="center", va="center",
        bbox=dict(boxstyle="round,pad=0.1", facecolor="white",
                  edgecolor=ego_color, alpha=0.9),
        zorder=32,
    )
    ...
    half = max(float(all_x.max() - all_x.min()),
               float(all_y.max() - all_y.min())) * 0.6 + 6.0
    ...
    ax_ts.plot(t_axis, per_ts_min, "-", color="#333", lw=1.3, alpha=0.9,
               label="min clearance to any stopped NPC")
    ax_ts.plot(t_axis[argmin_t], per_ts_min[argmin_t], "o",
               color=ego_color, ms=7, zorder=10,
               markeredgecolor="white", markeredgewidth=0.7,
    ...
    ax_ts.set_ylim(min(-0.2, float(per_ts_min.min()) - 0.1),
...
def _draw_timeline_figure(summary: list[dict], output_path: Path, viz_threshold_m: float) -> None:
    ...
    ax.plot(steps, dmin, "-", color="#333", lw=1.0, alpha=0.8,
            label="predicted sc_min_dist")
    ax.plot(steps[crossed], dmin[crossed], "o", color="#cc0000", ms=4,
            label=f"crossing (d<0.2 m): {int(crossed.sum())}")
    ax.axhline(viz_threshold_m, color="#777", lw=0.8, ls="--", alpha=0.6,
...
def main() -> None:
    ...
    parser.add_argument("--run_dir", type=Path, required=True,
                        help="Replay run directory (must contain npz/ and "
                             "spawn_config.json).")
    parser.add_argument("--model_path", type=Path, required=True,
                        help="Model checkpoint used for inference.")
    parser.add_argument("--config", type=Path, required=True,
                        help="Reward config JSON. MUST have "
                             "static_collision_enabled=true.")
    parser.add_argument("--output_dir", type=Path, default=None,
                        help="Where to write summary.json + step_NNNN.png "
                             "(default: <run_dir>/static_audit/).")
    parser.add_argument("--viz_threshold", type=float, default=2.0,
                        help="Render overlay PNG only when predicted "
                             "sc_min_dist < this (metres). Default 2.0.")
    parser.add_argument("--device", type=str, default="cuda")
    parser.add_argument("--stride", type=int, default=1,
                        help="Process every Nth sim step. Default 1 (all).")
    ...
    for i, path in enumerate(npz_paths):
        ...
        if result["sc_min_dist"] < args.viz_threshold:
            _draw_audit_figure(
                data_np, step, result["_arrays"],
                result["viz_d"], result["viz_t"],
