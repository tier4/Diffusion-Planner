...
def classify(
    etas: dict[str, float], lat_thresh: float, col_thresh: float, rule: str
) -> tuple[bool, list[str]]:
...
def main():
    ...
    parser.add_argument(
        "--model_path",
        required=True,
        help="frozen base planner checkpoint (must be the model the policy was trained against)",
    )
    parser.add_argument(
        "--policy_dir",
        required=True,
        help="dir with exploration_policy.pth + exploration_policy_config.json",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.15,
        help="min |eta_collision| to count as an avoidance request",
    )
    parser.add_argument(
        "--rule",
        choices=["any", "both"],
        default="any",
        help="'any': either head over threshold flags the scene; 'both': require both heads",
    )
    parser.add_argument(
        "--batch_size", type=int, default=32, help="scenes per batched det+encoder+policy pass"
    )
    parser.add_argument(
        "--render_only_avoidance",
        action="store_true",
        help="with --render_dir: render only flagged (and clearance-demoted) scenes, skip normals",
    )
    parser.add_argument(
        "--out_avoidance_list", default=None, help="optional JSON list of flagged NPZ paths"
    )
    parser.add_argument(
        "--out_normal_list", default=None, help="optional JSON list of non-flagged NPZ paths"
    )
    parser.add_argument(
        "--render_dir",
        default=None,
    ...
    if render_dir or args.verify_clearance > 0:
        if not args.ego_shape:
            raise ValueError(
                "--render_dir / --verify_clearance require --ego_shape WB,L,W — no default"
            )
    ...
    for start in range(0, len(paths), args.batch_size):
        ...
        stack_in = [
            {k: v for k, v in d.items() if k not in ("ego_agent_future", "neighbor_agents_future")}
            for d in datas
        ]
        batch = _stack_scene_data(stack_in, device)
        norm_batch = _normalize_batch(batch, model_args)
        det_b = det_inference_batched(
        ...
        for i, sp in enumerate(batch_paths):
            ...
            det = det_np[i]
            is_avoid, triggers = classify(etas, args.lat_thresh, args.col_thresh, args.rule)
            row = {
                "scene": sp,
            ...
            if is_avoid and args.verify_clearance > 0:
                ...
                det_clr = (
                    plan_static_clearance(det, boxes, args.ego_shape_t, device)
                    if boxes
                    else float("inf")
                )
            ...
            if render_dir and (is_avoid or row.get("demoted") or not args.render_only_avoidance):
                # pool-prefix the PNG name (same-basename scenes across pools)
                cls = "avoid" if is_avoid else "demoted" if row.get("demoted") else "normal"
                png = render_dir / (f"{cls}__{Path(sp).parent.name}__{Path(sp).stem}.png")
                label = (
                    f"DEMOTED (det clearance {row['det_clearance']}m)" if cls == "demoted" else None
                )
                # per-scene norm_data for the guided-trajectory render
                nd = {
                    k: v.clone() if isinstance(v, torch.Tensor) else v for k, v in datas[i].items()
                }
                ...
                render_verdict(
                    ...
                    det,
                    nd,
                    is_avoid or bool(row.get("demoted")),
                    triggers,
                    args,
                    device,
                    png,
    ...
    if render_dir:
        ...
        print(f"[render] {len(list(render_dir.glob('*__*.png')))} PNGs + collages -> {render_dir}")
    ...
    print(
        ...
        + (
            f", {n_demoted} demoted by det clearance >={args.verify_clearance}m"
            if args.verify_clearance > 0
            else ""
        )
    )
    for h, st in summary["abs_eta_distribution"].items():
        if st:
            print(f"  |eta_{h}|: mean {st['mean']} p50 {st['p50']} p95 {st['p95']} max {st['max']}")
