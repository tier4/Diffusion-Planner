...
def _extract_accel_y(bag_dir):
    ...
    if row is None:
        con.close()
        raise SystemExit(
            f"no /localization/acceleration in {bag_dir} — needed for the measured "
    ...
    if not msgs:
        raise SystemExit(
            f"/localization/acceleration in {bag_dir} has ≤1 message — "
...
def compute_dynamics(
    speed, yaw_rate, dt: float, lat_accel_meas=None, max_jump_speed: float = 30.0
    ...
    yaw_rate = np.asarray(yaw_rate, np.float64)
    T = len(speed)
    pad = SG_WINDOW // 2
    lat_accel_kin = np.abs(yaw_rate * speed)
    lat_accel = (
        np.abs(np.asarray(lat_accel_meas, np.float64))
        if lat_accel_meas is not None
        else lat_accel_kin
    )
    ...
    out = {
        "lat_accel": lat_accel,
        "lat_accel_kin": lat_accel_kin,
        "jerk": np.asarray(jerk, np.float64),
        "yaw_rate": np.abs(yaw_rate),
        "curvature": curvature,
        "speed": np.abs(speed),
    }
    for k in out:
...
def analyze_bag(bag, route_obj, pts, arc, args):
    ...
    dyn, glitch = compute_dynamics(
        speed, yaw_rate, dt, lat_accel_meas=lat_meas, max_jump_speed=args.max_jump_speed
    )
    ...
    if fin.sum() > 2:
        ...
        print(
            f"  [{Path(bag).name}] accel.y vs v·ω: corr={cc:.3f} max|Δ|={d.max():.3f} "
            f"(≈0 in psim; >0 on a real bag = slip/noise)"
        )

    arc_max = float(arc.max())
    proj = np.array(
        [project_to_polyline(np.array([float(px), float(py)]), pts, arc) for px, py in zip(x, y)]
    )
    pose_arc = proj[:, 0]
    ...
    print(
        f"  [{Path(bag).name}] {len(x)} samples (dt={dt:.3f}s), "
    ...
    if args.probe_lo is not None:
        ...
        print(
            f"  PROBE {Path(bag).name} arc [{args.probe_lo:.0f},{args.probe_hi:.0f}) "
            f"n={int(pm.sum())}: arc | lat_signed | lat_acc | jerk | yaw_rate | speed"
        )
        for i in np.where(pm)[0]:
            print(
                f"    {pose_arc[i]:7.1f} | {proj[i, 1]:+6.2f} | {dyn['lat_accel'][i]:6.2f} | "
    ...
    res = {
        ...
        "dt": dt,
        "bins": {},
    }  # dt = the REAL per-bag dt actually used (from timestamps)
    ...
    cl_mean = np.full(n_bins, np.nan)
    cl_std = np.full(n_bins, np.nan)
    cl_max = np.full(n_bins, np.nan)
    n_samp = np.zeros(n_bins, dtype=int)
    al_k = abs_lat[keep]
    sl_k = signed_lat[keep]
    for b in range(n_bins):
    ...
    res["bins"]["cl"] = {
        "mid": ((np.arange(n_bins) + 0.5) * args.bin_m).tolist(),
        "mean": cl_mean.tolist(),
        "std": cl_std.tolist(),
        "max": cl_max.tolist(),
        "n": n_samp.tolist(),
...
def print_compare_table(route_name, base, model, args):
    ...
    print(
        "Columns: latacc p95/max (m/s²) | jerk p95 (m/s³) | yawrate max (rad/s) | cl mean/std (m). std = lateral oscillation."
    )
    hdr = (
        "| arc | b_latacc_p95 | m_latacc_p95 | b_latacc_max | m_latacc_max "
    ...
    for i, mid in enumerate(mids):
        ...
        mn = g(model, "cl", "n")
        row = [
            f"{lo}-{lo + int(args.bin_m)}",
            _fmt(g(base, "lat_accel", "p95")),
            _fmt(g(model, "lat_accel", "p95")),
            _fmt(g(base, "lat_accel", "max")),
            _fmt(g(model, "lat_accel", "max")),
            _fmt(g(base, "jerk", "p95")),
            _fmt(g(model, "jerk", "p95")),
            _fmt(g(base, "yaw_rate", "max")),
            _fmt(g(model, "yaw_rate", "max")),
            _fmt(g(base, "cl", "std")),
            _fmt(g(model, "cl", "std")),
    ...
    for i, mid in enumerate(mids):
        ...
        if None not in (bs, ms, bc, mc, bl, ml) and (bl > 0.3 or ml > 0.3):
            ...
            if np.isfinite(sp_r) and np.isfinite(cu_r):
                tag = (
                    "SPEED"
                    if sp_r > 1.15 and sp_r > cu_r
                    else ("CURV" if cu_r > 1.15 and cu_r > sp_r else "~")
                )
        print(
            "| "
            + " | ".join(
...
def plot_compare(route_obj, pts, arc, base, model, metric, stat, out_png, args):
    ...
    plot_route_heatmap(
        axes[0], pts, segs, bvals, f"baseline {metric} {stat}", vmin, vmax, "inferno"
    )
    plot_route_heatmap(
        axes[1], pts, segs, mvals, f"{args.label} {metric} {stat}", vmin, vmax, "inferno"
    )
...
def main():
    ...
    ap.add_argument(
        "--ego_shape", required=True, help="WB,L,W (only printed; dynamics need no OBB)"
    )
    ...
    print(
        f"[comfort] route={Path(args.route).name} stride={args.stride} (dt derived from timestamps) "
    ...
    result = {
        ...
        "dt": model["dt"],
        "bin_m": args.bin_m,
        "model": {"label": args.label, "bag": args.bag, **model},
    }
    ...
    if args.baseline_bag:
        ...
        for metric, stat in (("lat_accel", "p95"), ("jerk", "p95"), ("cl", "std")):
            plot_compare(
                ...
                metric,
                stat,
                out / f"comfort_{Path(args.route).stem}_{metric}_{stat}.png",
                args,
            )
