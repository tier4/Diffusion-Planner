...
def score_rollout(rollout, boxes, ego_shape, device, dt=0.1):
    ...
    traj = np.stack([pos[1:, 0], pos[1:, 1], np.cos(pos[1:, 2]), np.sin(pos[1:, 2])], axis=-1)
    min_clear = plan_static_clearance(traj.astype(np.float32), boxes, ego_shape, device)
...
def main():
    ...
    for sp in paths:
        try:
            ...
            r_base = closed_loop_rollout_with_plans(
                model, margs, data, n_steps=args.steps, trim_backward=args.trim_backward
            )
        ...
        print(
            f"  {row['scene']:26s} base[clr={b['min_clear']:+.2f} "
    ...
    def agg(key):
        ...
        return {
            ...
            "min_clear_p5": round(float(np.percentile([s["min_clear"] for s in sub], 5)), 3),
        }
    ...
    print(json.dumps({k: report[k] for k in ("n", "baseline", "explorer")}, indent=1))
