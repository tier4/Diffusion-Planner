...
def _draw_box(ax, corners, color, crossing, z):
    ...
    ax.plot(
        ...
        poly[:, 1],
        "-",
        color=(CROSS_C if crossing else color),
        lw=(3 if crossing else 1.5),
        zorder=z + 1,
    )
...
def main():
    ...
    ap.add_argument(
        "--front_cut", type=float, default=50.0, help="skip first N m of route (ends not in-bounds)"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=50.0,
        help="spatial pad (m) when pre-filtering lanes/borders to crossing regions",
    )
    ...
    if not s1:
        raise SystemExit(
            f"map {route.map_path} has no road-border polylines (>=2 points) — cannot render RB crossings"
        )
    ...
    xa = sorted(
        [
            r[3]
            for r in base + mod
            if r[4] < thresh and args.front_cut <= r[3] <= amax - args.tail_cut
        ]
    )
    if not xa:
        print("no in-bounds crossings to render")
        return
    windows = []
    lo = xa[0]
    prev = xa[0]
    ...
    if len(region_xy):
        ...
        inbox = lambda pl: (
            pl.shape[0] >= 2
            and (
                (pl[:, 0] >= xmin) & (pl[:, 0] <= xmax) & (pl[:, 1] >= ymin) & (pl[:, 1] <= ymax)
            ).any()
        )
    ...
    for wi, (wlo, whi) in enumerate(windows):
        ...
        if not frames:
            print(
                f"window {wi}: arc {int(wlo)}-{int(whi)}m has no model poses in [{a0:.0f},{a1:.0f}]m — skipping"
            )
            continue
        wd = out / f"win{wi}_arc{int(wlo)}-{int(whi)}"
        ...
        for fi, mr in enumerate(frames):
            ...
            if ll:
                ax.add_collection(
                    LineCollection(ll, colors="#999999", linewidths=0.8, alpha=0.45, zorder=2)
                )
            bl = [pl for pl in borders if inview(pl)]
            if bl:
                ax.add_collection(
                    LineCollection(bl, colors=NB_BORDER, linewidths=2.2, alpha=0.9, zorder=5)
                )
            ...
            ax.plot(
                [], [], "-", color=MODEL_C, lw=2, label=f"{args.model_label}  border={mr[4]:.2f}m"
            )
            ax.set_xlim(cx - args.view_half, cx + args.view_half)
            ax.set_ylim(cy - args.view_half, cy + args.view_half)
            ax.set_aspect("equal")
            ax.grid(True, alpha=0.15)
            ax.legend(fontsize=10, loc="upper left")
            ax.set_title(
                f"RB crossing region  arc~{int(ma)}m   (red outline = ego crossing border, thresh {thresh:.2f}m)",
                fontsize=11,
            )
            fig.tight_layout()
            fig.savefig(wd / f"f{fi:04d}.png", dpi=100)
            fig.clf()
