...
def compute_outer_with_outward(data):
    ...
    return {
        "all_p1": s1, "all_p2": s2,
        "outer_p1": s1[is_outer], "outer_p2": s2[is_outer],
...
def plot(traj_t, data, ego_shape, info, out_path, target_t, scene_name, traj_label,
         lane_cross_thresh):
    ...
    for i in range(p1.shape[0]):
        if o_m[i]:
            ax.plot([p1[i, 0], p2[i, 0]], [p1[i, 1], p2[i, 1]],
                    color="black", linewidth=2.2, alpha=0.9, zorder=5)
        elif j_m[i]:
            ax.plot([p1[i, 0], p2[i, 0]], [p1[i, 1], p2[i, 1]],
                    color="darkorange", linewidth=2.5, alpha=0.9, zorder=6)
        elif s_m[i]:
            ax.plot([p1[i, 0], p2[i, 0]], [p1[i, 1], p2[i, 1]],
                    color="steelblue", linewidth=0.7, alpha=0.4, zorder=4)
    ...
    ax.add_patch(MplPolygon(world_corners, closed=True, fill=False,
                            edgecolor="darkgreen", linewidth=3, zorder=10,
    ...
    for p in range(wp.shape[0]):
        ...
        ax.scatter(wp[p, 0], wp[p, 1], c=col, s=55, marker="X" if col == "red" else "o",
                   zorder=11, edgecolors="black", linewidths=0.4)

    # Draw line to closest outer segment for the most-"crossed" point
    worst = int(sd.argmax())
    ax.plot([wp[worst, 0], closest_np[worst, 0]],
            [wp[worst, 1], closest_np[worst, 1]],
            color="red", linewidth=2.5, linestyle="-", zorder=12)
    ax.annotate(f"worst signed dist = {sd[worst]:+.3f}m (unsigned {ud[worst]:.3f}m)",
                xy=(wp[worst, 0], wp[worst, 1]),
                xytext=(wp[worst, 0] + 1.5, wp[worst, 1] + 1.5),
                fontsize=10, weight="bold", color="red",
                arrowprops=dict(arrowstyle="->", color="red", lw=1.2), zorder=13)
    ...
    ax.plot([ws_p1[0], ws_p2[0]], [ws_p1[1], ws_p2[1]],
            color="red", linewidth=4, alpha=0.7, zorder=9,
    ...
    title = (f"{scene_name} — traj={traj_label}, t={target_t}\n"
    ...
    print(f"  Perimeter signed dists: min={sd.min():+.3f}, max={sd.max():+.3f}, mean={sd.mean():+.3f}")
...
def main():
    ...
    if info is None:
        print("No outer segments found"); sys.exit(1)

    scene_name = Path(valid_paths[target_idx]).stem
    traj_label = f"idx={args.traj_idx}"
    plot(traj, data, ego_shape, info, args.output, args.target_t, scene_name, traj_label,
