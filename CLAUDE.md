...
- **Guidance system** (`model/guidance/`): Pluggable guidance functions (road_border, collision, lane_keeping, centerline, route, speed, lateral, longitudinal) applied during diffusion sampling via `guidance_wrapper.py`. Speed guidance supports a `stretch` param (>1 = faster, <1 = slower) using surrogate gradients. Longitudinal guidance (PlannerRFT Eq. 3) is broken for speed control — only useful for tiny exploration perturbations.
...
- **Reward**: `reward.py` scores trajectories on safety (collision, border, lane departure), progress, smoothness, feasibility, centerline following. Collision detection uses OBB signed distance with rear-end suppression: if an NPC overlaps the ego from behind at any timestep, that NPC is excluded from collision checks at all subsequent timesteps (prevents false positives when a rear-ending NPC passes the ego and enters the forward hemisphere). Lane departure uses polygon containment + midpoint nudge to classify road edges. Road border uses point-to-segment distance (consecutive valid border polyline points form segments). Both support configurable thresholds and exclusive zone categories (crossing > near > wide > safe). Config: `rb_near_scale`/`rb_wide_scale`/`rb_cont_scale` for RB penalty weights, `rb_gate_enabled` controls hard safety gate, `rb_cross_thresh`/`rb_near_thresh`/`rb_wide_thresh`/`rb_cont_thresh` for distance thresholds (defaults: 0.20/0.45/0.60/1.00m, offset +20cm vs lane). Lane thresholds: `lane_near_thresh`/`lane_wide_thresh`/`lane_cont_thresh` (defaults: 0.25/0.40/0.80m). Old config field names (`near_edge_scale` etc.) auto-mapped via `GRPOConfig.from_json`.
...
- **Ranked SFT**: `grpo_sft_trainer.py` generates N trajectories, selects best by reward, applies Savitzky-Golay filter, trains with standard SFT diffusion loss including neighbor outputs. Config: `ranked_sft_mode` = "gt_neighbor" or "baseline_neighbor". Supports **neighbor regularization** (`neighbor_reg_weight`): MSE(lora_neighbor, base_neighbor) prevents neighbor L2 drift through shared DiT weights. `neighbor_reg_only=true` drops the neighbor SFT loss for a cleaner signal. `neighbor_reg_weight` must be ≥ 1.0 at lr=5e-4 (0.5 and 0.75 diverge at ep5-6). Supports **ego IL regularization** (`ego_il_weight`): MSE(model_ego, GT/baseline_ego) anchors ego predictions. `ego_il_mode`: "gt" (GT anchor) or "baseline" (base model anchor, reuses neighbor_reg forward pass). Supports **selective training** (`selective_threshold`): only backprop on scenes where best-of-K reward exceeds deterministic by threshold. `selective_frozen=true` locks the scene set from epoch 1 to prevent oscillation. `selective_mode`: "threshold" (binary) or "advantage" (smooth weighting). **500sc frozen selective recipe:** `selective_frozen=true, selective_threshold=3.0, ga=16`. LR controls trade-off: lr=5e-4 → 0/50 LD at ep9 (+4.4% ego), lr=1e-4 → 0/50 at ep28 (+2.1% ego). Note: frozen selective trains only ~45 problem scenes per epoch (equivalent to 50sc); 450 normal scenes generate trajectories but get zero gradient. For genuine 500sc training, use standard RSFT without selective — achieves 0/50 LD but with high L2 cost (+113%). Generation uses CL+SPD guidance (hardcoded) + optional lateral/longitudinal guidance (schedulable). **IMPORTANT**: Config MUST explicitly set `n_prob_scenes` and `n_normal_scenes` — omitting these raises an error to prevent silent scene duplication.
- **Per-epoch scheduling**: `GRPOConfig.schedules` dict enables smooth interpolation of reward weights and guidance parameters over training epochs. Supports `linear`, `cosine`, `step` types. `linear` supports optional `end_epoch` to complete the ramp early then hold. Used for speed stretch scheduling during ranked SFT. Example: `"schedules": {"speed_stretch": {"type": "linear", "start": 1.0, "end": 1.2, "end_epoch": 8}}`
- **Exploration policy**: `grpo_exploration_trainer.py` learns per-scene lateral/longitudinal guidance (η_lat, η_lon) from Beta distributions. Supports `exploration_loss_type`: `"advantage_logprob"` (advantage-weighted log_prob of sampled etas) or `"best_sample_mse"` (MSE regression toward best-reward eta). Train on 500 diverse scenes for scene-dependent behavior — 50-scene training only learns a global bias.
...
- `lora_target` controls which blocks: "all", "first" (block 0), "last", "blocks01", "blocks02" (skip block 1)
- Load with `load_lora_checkpoint()` — never use `PeftModel.from_pretrained` directly
- **Block ablation**: zeroing LoRA blocks post-training can improve L2. With neighbor reg, block 2 is critical for lane keeping (removing it → 22/50 LD). Block 0 and block 1 are safe to zero — **no_block1 gives best ego L2** while preserving both LD and neighbor L2. Training only block 2 (`lora_target="last"`) diverges by ep9 — always train all blocks and ablate post-hoc.
