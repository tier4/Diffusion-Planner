"""Visualize per-scene Fusion attention over every encoder token class.

Spatial tokens are overlaid on a bird's-eye view at the position used by the
encoder. Non-spatial tokens remain visible in the Top-K chart and JSON output.
This is separate from the neighbor-only visualization and the dataset-level
aggregate analysis.
"""

import argparse
import json
from pathlib import Path

import matplotlib

matplotlib.use("Agg", force=True)

import matplotlib.pyplot as plt
import numpy as np
import torch
from diffusion_planner.train_epoch import heading_to_cos_sin
from diffusion_planner.utils.dataset import DiffusionPlannerData
from diffusion_planner.utils.visualize_input import visualize_inputs
from matplotlib.lines import Line2D
from torch.utils.data import DataLoader, Subset
from visualize_neighbor_attention import (
    attention_for_sample,
    candidate_indices,
    find_fusion,
    layer_indices,
    load_encoder,
    movement_and_turn,
    patch_fusion,
    prepare_inputs,
    sample_to_batch,
)

TOKEN_CLASSES = (
    "ego",
    "neighbors",
    "static",
    "lanes",
    "route",
    "polygons",
    "line_strings",
    "goal_pose",
    "ego_shape",
    "turn_indicator",
)

SPATIAL_CLASSES = (
    "ego",
    "neighbors",
    "static",
    "lanes",
    "route",
    "polygons",
    "line_strings",
    "goal_pose",
)

CLASS_MARKERS = {
    "ego": "*",
    "neighbors": "o",
    "static": "X",
    "lanes": "s",
    "route": "D",
    "polygons": "h",
    "line_strings": "^",
    "goal_pose": "P",
}


def token_layout(sample: dict) -> list[tuple[str, int, int]]:
    """Return class name and [start, end) offsets in Fusion token order."""
    counts = {
        "ego": 1,
        "neighbors": int(np.asarray(sample["neighbor_agents_past"]).shape[0]),
        "static": int(np.asarray(sample["static_objects"]).shape[0]),
        "lanes": int(np.asarray(sample["lanes"]).shape[0]),
        "route": int(np.asarray(sample["route_lanes"]).shape[0]),
        "polygons": int(np.asarray(sample["polygons"]).shape[0]),
        "line_strings": int(np.asarray(sample["line_strings"]).shape[0]),
        "goal_pose": 1,
        "ego_shape": 1,
        "turn_indicator": 1,
    }
    layout = []
    offset = 0
    for class_name in TOKEN_CLASSES:
        end = offset + counts[class_name]
        layout.append((class_name, offset, end))
        offset = end
    return layout


def all_token_attention(store, layer: str) -> torch.Tensor:
    """Return [B, token_count] attention from the ego query to every key."""
    selected = layer_indices(layer, len(store))
    return torch.stack([store[index]["weights"][:, 0] for index in selected]).mean(dim=0)


def class_slice(layout, class_name: str) -> slice:
    for name, start, end in layout:
        if name == class_name:
            return slice(start, end)
    raise KeyError(class_name)


def representative_position(sample: dict, class_name: str, local_index: int):
    """Return the raw position represented by a Fusion token, if it has one."""
    if class_name == "ego":
        return 0.0, 0.0
    if class_name == "neighbors":
        point = np.asarray(sample["neighbor_agents_past"])[local_index, -1]
        return float(point[0]), float(point[1])
    if class_name == "static":
        point = np.asarray(sample["static_objects"])[local_index]
        return float(point[0]), float(point[1])
    if class_name == "lanes":
        points = np.asarray(sample["lanes"])[local_index]
        point = points[points.shape[0] // 2]
        return float(point[0]), float(point[1])
    if class_name == "route":
        points = np.asarray(sample["route_lanes"])[local_index]
        point = points[points.shape[0] // 2]
        return float(point[0]), float(point[1])
    if class_name == "polygons":
        points = np.asarray(sample["polygons"])[local_index]
        point = points[points.shape[0] // 2]
        return float(point[0]), float(point[1])
    if class_name == "line_strings":
        points = np.asarray(sample["line_strings"])[local_index]
        point = points[points.shape[0] // 2]
        return float(point[0]), float(point[1])
    if class_name == "goal_pose":
        point = np.asarray(sample["goal_pose"])
        return float(point[0]), float(point[1])
    return None


def neighbor_subclass(sample: dict, local_index: int) -> str:
    point = np.asarray(sample["neighbor_agents_past"])[local_index, -1]
    names = ("vehicle", "pedestrian", "bicycle")
    return names[int(np.argmax(point[8:11]))]


def token_records(sample: dict, scores: np.ndarray, valid: np.ndarray) -> list[dict]:
    """Create one JSON-ready record for every valid Fusion token."""
    records = []
    for class_name, start, end in token_layout(sample):
        for token_index in range(start, end):
            if not valid[token_index]:
                continue
            local_index = token_index - start
            position = representative_position(sample, class_name, local_index)
            record = {
                "token_index": token_index,
                "class": class_name,
                "class_index": local_index,
                "attention": float(scores[token_index]),
                "attention_pct_all_tokens": float(scores[token_index] * 100.0),
                "spatial": position is not None,
                "x_m": float(position[0]) if position is not None else None,
                "y_m": float(position[1]) if position is not None else None,
            }
            if position is not None:
                record["distance_m"] = float(np.hypot(*position))
            if class_name == "neighbors":
                record["neighbor_class"] = neighbor_subclass(sample, local_index)
            records.append(record)
    return sorted(records, key=lambda item: item["attention"], reverse=True)


def class_summary(records: list[dict]) -> dict:
    result = {}
    for class_name in TOKEN_CLASSES:
        selected = [record for record in records if record["class"] == class_name]
        result[class_name] = {
            "valid_count": len(selected),
            "attention_pct_all_tokens": float(
                sum(record["attention_pct_all_tokens"] for record in selected)
            ),
        }
    return result


def select_scene(
    encoder,
    cfg,
    fusion_store,
    dataset,
    indices: list[int],
    select_class: str,
    layer: str,
    batch_size: int,
    device: str,
) -> tuple[int, float]:
    """Choose the scene containing the strongest eligible token."""
    loader = DataLoader(Subset(dataset, indices), batch_size=batch_size, shuffle=False)
    reference_layout = token_layout(dataset[indices[0]])
    eligible_slice = (
        slice(0, reference_layout[-1][2])
        if select_class == "any"
        else class_slice(reference_layout, select_class)
    )
    best_index = None
    best_score = -1.0
    offset = 0
    with torch.no_grad():
        for raw in loader:
            fusion_store.clear()
            encoder(prepare_inputs(dict(raw), cfg, device))
            scores = all_token_attention(fusion_store, layer)
            valid = ~fusion_store[0]["mask"]
            eligible = torch.zeros_like(valid)
            eligible[:, eligible_slice] = True
            masked_scores = scores.masked_fill(~(valid & eligible), -1.0)
            batch_scores = masked_scores.max(dim=1).values
            local = int(batch_scores.argmax())
            score = float(batch_scores[local])
            if score > best_score:
                best_score = score
                best_index = indices[offset + local]
            offset += raw["neighbor_agents_past"].shape[0]
    if best_index is None or best_score < 0:
        raise RuntimeError(f"no valid {select_class} token found in candidate scenes")
    return best_index, best_score


def draw_report(
    batch: dict,
    records: list[dict],
    sample_index: int,
    sample_path: str,
    movement_m: float,
    turn_angle_deg: float,
    layer: str,
    top_k: int,
    view_range: float,
    colormap: str,
    marker_size_min: float,
    marker_size_max: float,
    out_png: Path,
    attention_vmax: float | None = None,
    attention_label: str = "Fusion attention from ego query",
    title_prefix: str = "All-token Attention Overlay",
    attention_layer_label: str | None = None,
    prediction=None,
    show_prediction: bool = False,
):
    fig, (scene_ax, rank_ax) = plt.subplots(
        1, 2, figsize=(19, 10), gridspec_kw={"width_ratios": [4.2, 1.15]}
    )
    visual_inputs = {key: value.clone() for key, value in batch.items()}
    visual_inputs["ego_agent_past"] = heading_to_cos_sin(visual_inputs["ego_agent_past"])
    visual_inputs["goal_pose"] = heading_to_cos_sin(visual_inputs["goal_pose"])
    visualize_inputs(visual_inputs, ax=scene_ax, view_ranges=[view_range])
    for text_artist in list(scene_ax.texts):
        text_artist.remove()
    if show_prediction and prediction is not None:
        from visualize_prediction_overlay import draw_prediction_paths

        draw_prediction_paths(scene_ax, batch, prediction, ego=True, neighbors=True)

    values = np.asarray([record["attention_pct_all_tokens"] for record in records])
    vmax = max(
        attention_vmax
        if attention_vmax is not None
        else (float(values.max()) if values.size else 0.0),
        1e-9,
    )
    norm = matplotlib.colors.Normalize(vmin=0.0, vmax=vmax)
    cmap = matplotlib.colormaps[colormap]

    for class_name in SPATIAL_CLASSES:
        selected = [
            record
            for record in records
            if record["class"] == class_name
            and abs(record["x_m"]) <= view_range
            and abs(record["y_m"]) <= view_range
        ]
        if not selected:
            continue
        class_values = np.asarray([record["attention_pct_all_tokens"] for record in selected])
        scene_ax.scatter(
            [record["x_m"] for record in selected],
            [record["y_m"] for record in selected],
            s=marker_size_min + (marker_size_max - marker_size_min) * class_values / vmax,
            c=class_values,
            cmap=cmap,
            norm=norm,
            marker=CLASS_MARKERS[class_name],
            alpha=0.72,
            edgecolors="black",
            linewidths=0.55,
            zorder=20,
        )

    visible_spatial = [
        record
        for record in records
        if record["spatial"]
        and abs(record["x_m"]) <= view_range
        and abs(record["y_m"]) <= view_range
    ]
    for rank, record in enumerate(visible_spatial[:top_k], start=1):
        scene_ax.annotate(
            f"{rank}: {record['class']}[{record['class_index']}] "
            f"{record['attention_pct_all_tokens']:.2f}%",
            (record["x_m"], record["y_m"]),
            xytext=(6, 7 + 9 * ((rank - 1) % 3)),
            textcoords="offset points",
            fontsize=7,
            weight="bold",
            color="black",
            bbox={"boxstyle": "round,pad=0.16", "fc": "white", "ec": "black", "alpha": 0.8},
            zorder=30,
        )

    colorbar = fig.colorbar(
        matplotlib.cm.ScalarMappable(norm=norm, cmap=cmap), ax=scene_ax, fraction=0.035, pad=0.02
    )
    colorbar.set_label(f"{attention_label} (% of all valid tokens)")
    legend = [
        Line2D(
            [0],
            [0],
            marker=CLASS_MARKERS[name],
            color="none",
            markerfacecolor="lightgray",
            markeredgecolor="black",
            markersize=8,
            label=name,
        )
        for name in SPATIAL_CLASSES
    ]
    scene_ax.legend(
        handles=legend,
        loc="lower left",
        title="Spatial token class",
        ncols=2,
        fontsize=8,
    )
    scene_ax.set_title(
        f"{title_prefix} — dataset index {sample_index}\n"
        f"ego-query, {attention_layer_label or f'Fusion layer={layer}'}, "
        f"movement={movement_m:.1f} m, "
        f"turn proxy={turn_angle_deg:.1f}°"
    )

    displayed = list(reversed(records[: min(top_k, len(records))]))
    labels = [
        f"#{record['token_index']} {record['class']}[{record['class_index']}]"
        for record in displayed
    ]
    chart_values = [record["attention_pct_all_tokens"] for record in displayed]
    colors = [cmap(norm(value)) for value in chart_values]
    rank_ax.barh(range(len(displayed)), chart_values, color=colors, edgecolor="black")
    rank_ax.set_yticks(range(len(displayed)), labels)
    rank_ax.set_xlabel(f"{attention_label} (% of all valid tokens)")
    rank_ax.set_title(f"Top {len(displayed)} tokens across all classes")
    rank_ax.grid(axis="x", alpha=0.25)
    rank_ax.tick_params(axis="both", labelsize=7)
    for row, value in enumerate(chart_values):
        rank_ax.text(value, row, f" {value:.3f}%", va="center", fontsize=7)
    rank_ax.text(
        0.0,
        -0.10,
        f"Source NPZ: {Path(sample_path).name}\n"
        "ego_shape / turn_indicator are non-spatial and appear only in this ranking.",
        transform=rank_ax.transAxes,
        fontsize=7,
        va="top",
    )

    out_png.parent.mkdir(parents=True, exist_ok=True)
    fig.tight_layout()
    fig.savefig(out_png, dpi=160, bbox_inches="tight")
    plt.close(fig)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Overlay ego-query Fusion attention for every encoder token class."
    )
    parser.add_argument("--run_dir", required=True)
    parser.add_argument("--valid_set_list", required=True)
    parser.add_argument("--sample_index", type=int, default=None)
    parser.add_argument(
        "--select_class",
        choices=("any", *TOKEN_CLASSES),
        default="any",
        help="class used only when automatically selecting a sample",
    )
    parser.add_argument("--candidate_count", type=int, default=128)
    parser.add_argument("--batch_size", type=int, default=4)
    parser.add_argument("--move_min_m", type=float, default=5.0)
    parser.add_argument("--turn_only", action="store_true")
    parser.add_argument("--turn_deg", type=float, default=15.0)
    parser.add_argument(
        "--layer", default="mean", help="'mean', 'last', or a zero-based Fusion layer index"
    )
    parser.add_argument("--top_k", type=int, default=20)
    parser.add_argument("--view_range", type=float, default=80.0)
    parser.add_argument("--colormap", default="plasma")
    parser.add_argument("--marker_size_min", type=float, default=25.0)
    parser.add_argument("--marker_size_max", type=float, default=700.0)
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--out_png", default="all_token_attention.png")
    parser.add_argument(
        "--out_json", default="", help="metadata path; defaults to the PNG path with .json suffix"
    )
    return parser.parse_args()


def main():
    args = parse_args()
    if args.colormap not in matplotlib.colormaps:
        raise ValueError(f"unknown Matplotlib colormap: {args.colormap}")
    if args.marker_size_min <= 0 or args.marker_size_max < args.marker_size_min:
        raise ValueError("--marker_size_max must be >= --marker_size_min > 0")

    dataset = DiffusionPlannerData(args.valid_set_list)
    encoder, cfg = load_encoder(Path(args.run_dir).resolve(), args.device)
    fusion_store = []
    patch_fusion(find_fusion(encoder), fusion_store)

    if args.sample_index is None:
        indices = candidate_indices(
            dataset,
            args.candidate_count,
            args.move_min_m,
            args.turn_only,
            args.turn_deg,
        )
        sample_index, selection_score = select_scene(
            encoder,
            cfg,
            fusion_store,
            dataset,
            indices,
            args.select_class,
            args.layer,
            args.batch_size,
            args.device,
        )
        print(
            f"selected dataset index {sample_index} from {len(indices)} candidates "
            f"(max {args.select_class} attention={selection_score:.6f})",
            flush=True,
        )
    else:
        if not 0 <= args.sample_index < len(dataset):
            raise IndexError(f"--sample_index must be in 0..{len(dataset) - 1}")
        sample_index = args.sample_index

    sample = dataset[sample_index]
    batch, _, _ = attention_for_sample(encoder, cfg, fusion_store, sample, args.layer, args.device)
    scores = all_token_attention(fusion_store, args.layer)[0].cpu().numpy()
    valid = (~fusion_store[0]["mask"][0]).cpu().numpy()
    records = token_records(sample, scores, valid)
    movement_m, turn_angle_deg = movement_and_turn(sample)
    sample_path = dataset.data_list[sample_index]
    out_png = Path(args.out_png).resolve()
    draw_report(
        batch,
        records,
        sample_index,
        sample_path,
        movement_m,
        turn_angle_deg,
        args.layer,
        args.top_k,
        args.view_range,
        args.colormap,
        args.marker_size_min,
        args.marker_size_max,
        out_png,
    )

    out_json = Path(args.out_json).resolve() if args.out_json else out_png.with_suffix(".json")
    report = {
        "sample_index": sample_index,
        "sample_path": sample_path,
        "movement_m": movement_m,
        "turn_angle_deg": turn_angle_deg,
        "layer": args.layer,
        "valid_token_count": len(records),
        "attention_sum_pct": float(sum(record["attention_pct_all_tokens"] for record in records)),
        "attention_definition": (
            "head-averaged Fusion self-attention from the ego-token query; "
            "selected layers are averaged"
        ),
        "position_definition": (
            "the representative position used by each encoder token; "
            "ego_shape and turn_indicator are non-spatial"
        ),
        "visualization": {
            "colormap": args.colormap,
            "marker_size_min": args.marker_size_min,
            "marker_size_max": args.marker_size_max,
            "top_k": args.top_k,
            "view_range_m": args.view_range,
        },
        "class_summary": class_summary(records),
        "tokens": records,
    }
    out_json.parent.mkdir(parents=True, exist_ok=True)
    with out_json.open("w") as file:
        json.dump(report, file, indent=2)
    print(f"wrote {out_png}", flush=True)
    print(f"wrote {out_json}", flush=True)


if __name__ == "__main__":
    main()
