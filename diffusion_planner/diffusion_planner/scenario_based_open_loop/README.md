...
- `lane_change` requires `lanes` (or `route_lanes`) with the boundary-offset columns (`D>=8`) and `ego_agent_future`: within the horizon the prediction must cross the source lane's own boundary in the direction the GT went, and end within half a lane width of the GT's final lateral position. Lateral position is measured against the source lane — the starting lanelet extended through its endpoint-chained successors — so the score is independent of longitudinal error, and lane width comes from the map rather than a constant. Scenes whose GT never leaves its own lane are rejected as mis-curated rather than scored.
