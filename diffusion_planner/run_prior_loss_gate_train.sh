#!/bin/bash
# SYNTHESIS + SCENE-AWARE GATE: same recipe as run_prior_loss_train.sh (prior-conditioning INPUT
# + consistency LOSS, warm-start ep60, lr 3e-5, batch 256) with --tc_scene_gate True. The
# consistency loss on each kept sample is weighted by w=exp(-gt_dev/tau), gt_dev = GT-vs-GT
# scene-change deviation: normal frames keep full consistency (stability), scene-change frames
# drop it so GT wins (responsiveness). Targets the synthesis failure (flk -34% but copied:
# responsiveness 0.29 vs base 0.62) by decoupling flicker-gain from copying.
set -ux
cd "$(dirname "$0")"
EXP_NAME=${1:-prior_loss_gate_train}
EPOCHS=${2:-5}
TAU=${3:-6.0}    # exp decay above the adaptive floor (calibrated on real gt_dev distribution)
QQ=${4:-0.5}     # adaptive floor = this quantile of gt_dev (frames <= it keep full consistency)
...
/mnt/nvme/OnePlanner/.venv/bin/python -m torch.distributed.run --nnodes 1 --nproc-per-node 8 --standalone \
  ...
  --tc_scene_gate True --tc_gate_tau "${TAU}" --tc_gate_q "${QQ}" \
