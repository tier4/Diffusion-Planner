...
def load_model(args, ckpt_path, use_ema, device):
    ...
    if missing or unexpected:
        print(
            f"[load_model] missing={len(missing)} unexpected={len(unexpected)} "
...
def draw_road(ax, raw_np, s):
    ...
    scene = {
        k: raw_np[k][s : s + 1]
        for k in ("lanes", "route_lanes", "line_strings", "polygons", "static_objects")
        if k in raw_np
    }
...
def draw_footprints(ax, traj_xyhead, length, width, color, stride, alpha=0.35, lw=0.8, mode="tail"):
...
def main():
    ...
    if v.aug_mode != "none":
        ...
        print(
            f"Augmentation '{v.aug_mode}' applied; "
    ...
    ego_samples = (
        ego_world.view(S, n, ego_world.shape[1], 4).cpu().numpy()
    )  # [S,N,T,4] (x,y,cos,sin)
    ...
    for s in range(S):
        ...
        for p in range(Pn):
            ...
            ax.plot(
                ...
                color=box_color,
                lw=box_lw,
                alpha=0.9 if is_injected else 0.7,
            ...
            if fut.shape[0] > 0:
                ax.plot(
                    ...
                    "-",
                    color=fut_color,
                    lw=1.2 if is_injected else 0.8,
                    alpha=0.9 if is_injected else 0.6,
                    zorder=3.5 if is_injected else 2,
                )
        ...
        if v.show_footprint:
            ...
            gt_xyh = np.stack(
                [gt_full[:, 0], gt_full[:, 1], np.cos(gt_full[:, 2]), np.sin(gt_full[:, 2])],
                axis=-1,
            )
            draw_footprints(
                ax,
            ...
            if v.annotate_reward:
                ...
                if end.shape[0] > 0:
                    ax.annotate(
                        ...
                        zorder=7,
                        bbox=dict(
                            boxstyle="round,pad=0.1", fc="white", ec=color, lw=0.8, alpha=0.7
                        ),
                    )
    ...
    print(
        f"  mean reward={mean_reward:.3f}  mean best-of-group={mean_best:.3f}"
