...
def parse_viz_args():
    ...
    p.add_argument("--scene_index", type=int, default=-1,
    ...
    p.add_argument("--zoom_margin", type=float, default=12.0,
...
def main():
    ...
    idx = v.scene_index if v.scene_index >= 0 else int(
        np.random.default_rng(v.seed).integers(len(dataset)))
    ...
    for j, t in enumerate(steps):
        ...
        rect = torch.stack([pos[:, 0], pos[:, 1], cos / norm_cs, sin / norm_cs,
                            neigh_length, neigh_width], dim=-1)  # [Pn, 6]
        ...
        for p in range(Pn):
            ...
            ax.add_patch(Polygon(neigh_corners_np[p], closed=True, fill=is_inj,
                                 facecolor=(c if is_inj else "none"),
                                 edgecolor=c, lw=lw, alpha=0.25 if is_inj else 0.7, zorder=3.5))
        ...
        for i in range(n):
            ...
            collided = pen_i > 0.0
            ax.add_patch(Polygon(ec, closed=True, fill=False, edgecolor=color,
                                 lw=2.2 if collided else 1.4, alpha=0.95, zorder=5))
            if collided:
                ax.add_patch(Polygon(ec, closed=True, fill=True, facecolor="red",
                                     edgecolor="red", lw=1.0, alpha=0.18, zorder=4.5))
                collide_list.append(f"s{i}(p={pen_i:.2f})")
            ax.annotate(str(i), xy=tuple(ec.mean(0)), fontsize=7, color=color,
                        ha="center", va="center", fontweight="bold", zorder=6)
            ax.plot(ego_xy[i, :, 0], ego_xy[i, :, 1], "-", color=color, lw=0.8, alpha=0.35, zorder=3)
        ...
        ax.set_title(
            ...
            fontsize=9, color=("red" if k_coll else "green"))
    ...
    sample_handles = [plt.Line2D([0], [0], color=scolor(i), lw=2,
                                 label=f"s{i} r={reward[i]:.1f}") for i in range(n)]
    proxies = [
    ...
    fig.legend(handles=sample_handles + proxies, loc="upper center",
               fontsize=8, ncol=min(n + 3, 11))
    ...
    print(f"  mean reward={reward.mean():.3f}  best={reward.max():.3f}  "
          f"total penalty={total_pen:.3f}")
